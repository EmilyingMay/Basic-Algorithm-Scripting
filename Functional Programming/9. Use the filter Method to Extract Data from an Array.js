...
// that function returns true. In other words, it filters the array,
...
// argument is the current element being processed. The second
// is the index of that element and the third is the array
// upon which the filter method was called.

// See below for an example using the filter method on the
// users array to return a new array containing only the users
// under the age of 30. For simplicity, the example only uses
...
// The variable watchList holds an array of
// objects with information on several movies.
// Use a combination of filter and map on watchList
// to assign a new array of objects with only title
// and rating keys. The new array should only include
// objects where imdbRating is greater than or equal to 8.0.
// Note that the rating values are saved as strings
// in the object and you may need to convert them into
...
var filteredList = watchList.map(
  (item) => (
    (title = item.Title),
    (imdbRating = item.imdbRating.filter(
      (item) => parseInt(item.imdbRating) > 8.0
    ))
  )
);
